...
def build_master_atlas(man,max_faces_each=60):
    ...
    for ip,p in enumerate(muscle_parts):
        ...
        Fs=np.asarray(F[::step,:3],np.int32)
        used=np.unique(Fs.reshape(-1))
        remap=np.full(len(V),-1,np.int32); remap[used]=np.arange(len(used),dtype=np.int32)
        Vs=V[used]
        V_all.append(Vs.astype(np.float32)); F_all.append(remap[Fs]+off)
...
def build_visible_human_muscle_sequence(man,joints,joint_names,skin_sequence,max_faces_each=60):
    """V110.3.22.5 master-atlas retargeting.
    1) Denver STL are cleaned once while preserving their common coordinates.
    2) One master atlas is registered to SKEL/OpenSim at frame 0.
    3) Frames 1..74 use rigid segment transforms + attachment-aware blend.
    No per-frame PCA and no independent re-fitting of individual STL meshes.
    """
    ...
    if master is None:
        return None,None
    Vbase,_=_map_master_frame0(master,J,nm,S[0])
    frames=[Vbase.astype(np.float32)]
    for t in range(1,len(J)):
        Vt=np.zeros_like(Vbase,dtype=float)
        for ip,name in enumerate(master["part_names"]):
            side=master["part_sides"][ip] if master["part_sides"][ip] in ("r","l") else "r"
            region=master["part_regions"][ip]
            chain=MUSCLE_CHAIN.get(name,(region,region))
            ...
            seg0,seg1=chain
            try:
                o00,ot0,R0=_rigid_target_delta(J,nm,t,side,seg0,None)
            except Exception:
                o00,ot0,R0=_rigid_target_delta(J,nm,t,side,region,None)
            try:
                o01,ot1,R1=_rigid_target_delta(J,nm,t,side,seg1,None)
            except Exception:
                o01,ot1,R1=o00,ot0,R0
            P0=ot0+(P-o00)@R0.T
            P1=ot1+(P-o01)@R1.T
            if seg0==seg1:
                W=P0
            else:
                # attachment-aware weights: endpoints follow their respective bodies.
                d0=np.linalg.norm(P-o00,axis=1)
                d1=np.linalg.norm(P-o01,axis=1)
                w=d0/(d0+d1+1e-9)
                w=np.clip(w,0.03,0.97)[:,None]
                W=(1-w)*P0+w*P1
            Vt[idx]=W
