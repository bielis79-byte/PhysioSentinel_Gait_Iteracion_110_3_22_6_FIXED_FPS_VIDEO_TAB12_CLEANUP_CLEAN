...
from integrated_viewer import build_hamner_sequence, muscle_paths_sequence, render_combined_mp4, render_layer_mp4, camera_preset, camera_from_angles, render_layer_mp4_manual_camera, render_layer_mp4_multiview
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.22.6-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.22.6 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.22.6 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        st.markdown("## 🧍🦴💪 Visor anatómico 3D · SKEL + OpenSim + Visible Human")
        ...
        if not _ham_status.get("ready"):
            ...
            _cache_key=(int(len(_Sv)),int(_Sv.shape[1]),float(np.nanmean(_Jv[0])),_vh_cache_sha)
            if st.session_state.get("v32110_seq_key")!=_cache_key:
                with st.spinner("Construyendo atlas maestro Denver→OpenSim en frame 1 y retargeteando los 75 frames SKEL…"):
                    ...
                    _BV,_BF=build_hamner_sequence(
                        ...
                        _build_opt,max_faces_each=150
                    )
                    _MSEQ,_MNAMES=muscle_paths_sequence(_Jv,_JNv,muscle_segments)
                    # V110.3.22.6: volumetric lower-limb muscle layer from Visible Human.
                    # Bones remain exclusively OpenSim/Hamner.
                    _vh_man=st.session_state.get("v3220_manifest")
                    _VMV=None; _VMF=None
                    if _vh_man and _vh_man.get("ready"):
                        try:
                            from visible_human_atlas import build_visible_human_muscle_sequence
                            _VMV,_VMF=build_visible_human_muscle_sequence(
                                _vh_man,_Jv,_JNv,_Sv,max_faces_each=60
                            )
                        except Exception as _ve:
                            st.warning(f"No se pudo preparar músculo volumétrico Visible Human: {type(_ve).__name__}: {_ve}")
                ...
                st.session_state["v3222_vh_muscle_vertices"]=_VMV
                st.session_state["v3222_vh_muscle_faces"]=_VMF
                st.session_state["v32110_seq_key"]=_cache_key

            _BV=np.asarray(st.session_state["v32110_bone_vertices"],dtype=np.float32)
            _BF=np.asarray(st.session_state["v32110_bone_faces"],dtype=np.int32)
            _MSEQ=st.session_state["v32110_muscle_seq"]
            _MNAMES=st.session_state["v32110_muscle_names"]
            _VMV=st.session_state.get("v3222_vh_muscle_vertices")
            _VMF=st.session_state.get("v3222_vh_muscle_faces")
            ...
            # Compact SKEL skin only for browser payload.
            _skin_step=max(1,int(np.ceil(len(_SF)/4200.0)))
            _SFd=_SF[::_skin_step]
            _used=np.unique(_SFd.reshape(-1))
            _rmap=np.full(_Sv.shape[1],-1,dtype=np.int32)
            _rmap[_used]=np.arange(len(_used),dtype=np.int32)
            _SFc=_rmap[_SFd]
            _SVc=_Sv[:,_used,:]
            ...
            _payload=_json.dumps({
                ...
                "BV":np.round(_BV,5).tolist(),"BF":_BF.tolist(),
                "M":_mus_frames,
                "VMV":np.round(np.asarray(_VMV,dtype=np.float32),5).tolist() if _VMV is not None else None,
                "VMF":np.asarray(_VMF,dtype=np.int32).tolist() if _VMF is not None else None,
...
let s=xyz(P.SV,0),b=xyz(P.BV,0),m=mus(0),vm=P.VMV?xyz(P.VMV,0):[[],[],[]];
const data=[
  {{type:'mesh3d',x:s[0],y:s[1],z:s[2],i:sfi,j:sfj,k:sfk,name:'Piel SKEL',
    opacity:.24,visible:true,flatshading:false,showscale:false}},
  {{type:'mesh3d',x:b[0],y:b[1],z:b[2],i:bfi,j:bfj,k:bfk,name:'Esqueleto',
    opacity:1,visible:true,flatshading:false,showscale:false}},
  {{type:'mesh3d',x:vm[0],y:vm[1],z:vm[2],i:vmfi,j:vmfj,k:vmfk,name:'Músculo Visible Human',
    opacity:.62,visible:!!P.VMV,flatshading:false,showscale:false}},
  {{type:'scatter3d',mode:'lines',x:m[0],y:m[1],z:m[2],name:'Músculo OpenSim funcional',
    line:{{width:7}},visible:!P.VMV}}
];
...
window.layer=function(mode){{
  ...
  Plotly.restyle(gd,{{visible:musOn && !!P.VMV}},[2]);
  Plotly.restyle(gd,{{visible:musOn && !P.VMV}},[3]);
}};

function frame(k){{
  n=(k+P.SV.length)%P.SV.length;
  s=xyz(P.SV,n);b=xyz(P.BV,n);m=mus(n);if(P.VMV)vm=xyz(P.VMV,n);
  Plotly.restyle(gd,{{x:[s[0]],y:[s[1]],z:[s[2]]}},[0]);
  Plotly.restyle(gd,{{x:[b[0]],y:[b[1]],z:[b[2]]}},[1]);
  if(P.VMV) Plotly.restyle(gd,{{x:[vm[0]],y:[vm[1]],z:[vm[2]]}},[2]);
...
async function renderFrameForExport(k,cam0,multi0){{
  ...
  if(P.VMV) jobs.push(Plotly.restyle(gd,{{x:[vm[0]],y:[vm[1]],z:[vm[2]]}},[2]));
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.22.6 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.22.6 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
